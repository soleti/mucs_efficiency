...
h_sig = TH1F("h_sig",";Significance [#sigma];N. Entries / 0.5",20,-10,10)

def measure_sig(val1,val2):
    return (val1-val2)/math.sqrt(err1**2+err2**2)
    
print("0 - Central, 1 - upstream, 2 - downstream")
with open("central_shift.txt") as textfile1, open("upstream_shift.txt") as textfile2, open("downstream_shift.txt") as textfile3: 
    for x, y, z in zip(textfile1, textfile2, textfile3):
        ...
        if eff[0] != 0 and eff[1] != 0:
            sig = (eff[0]-eff[1])/math.sqrt(err[0]**2+err[1]**2)
            h_sig.Fill(sig)
            if abs(sig) > 4:
                print(0,1,eff[0],eff[1],eff[2],a[0],a[1],a[2],sig)
        
        if eff[0] != 0 and eff[2] != 0:
            sig = (eff[2]-eff[0])/math.sqrt(err[0]**2+err[2]**2)
            h_sig.Fill(sig)
            if abs(sig) > 4:
                print(0,2,eff[0],eff[1],eff[2],a[0],a[1],a[2],sig)
                
        if eff[1] != 0 and eff[2] != 0:
            sig = (eff[1]-eff[2])/math.sqrt(err[1]**2+err[2]**2)
            h_sig.Fill(sig)
            if abs(sig) > 4:
                print(1,2,eff[0],eff[1],eff[2],a[0],a[1],a[2],sig)
